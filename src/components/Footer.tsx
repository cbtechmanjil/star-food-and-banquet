    ...
    <footer className="bg-foreground text-primary-foreground">
      <div className="container mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand */}
          <div>
            <img src={logoImg} alt="Star Banquet Pepsicola – Banquet Hall & Event Venue Kathmandu" className="h-16 w-auto object-contain mb-6" loading="lazy" />
          ...
          <div>
            ...
            <address className="not-italic flex flex-col gap-4 font-body text-sm text-primary-foreground/60">
              <div className="flex items-start gap-3">
                <MapPin className="w-4 h-4 mt-0.5 text-primary flex-shrink-0" />
                <span>{contact?.address || "Pepsicola, Kathmandu, Nepal"}</span>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="w-4 h-4 text-primary flex-shrink-0" />
                <a href={`tel:${contact?.phone || ""}`} className="hover:text-primary transition-colors">
                  {contact?.phone || "+977-XXXXXXXXXX"}
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="w-4 h-4 text-primary flex-shrink-0" />
                <a href={`mailto:${contact?.email || "info@starfoodbanquet.com"}`} className="hover:text-primary transition-colors">
                  {contact?.email || "info@starfoodbanquet.com"}
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Clock className="w-4 h-4 text-primary flex-shrink-0" />
                <span>{contact?.workingHours || "Sun–Fri: 9:00 AM – 9:00 PM"}</span>
              </div>
            </address>
          </div>
        </div>
